    ...
    for (var i = 0; i < arrayRFQAchieved.length; i++) {
     ...
     $('#tendersTurnover').fadeOut(300, function () {
        switch (category) {
            ...
            //    break;
            case 'rfq':
                 $('#tenderTurnoverTitle').text("Quotes");
...
function assignOnClickDataTable(id) {
    var tableSuppliers =  $('#' + id).DataTable({
        ...
        language: {
            info: " _START_ to _END_ from _TOTAL_ المجموع",
        }
    });
    ...
    //});
     $('#' + id + ' tbody').on('click', 'tr', function () {
        ...
        var totalApplied = parseInt( $(this).find(".applied").text());
         ...
         $('#txtTenderByUser').text("Tenders " +  $(this).find(".userName").text());
 ...
 $(document).ready(function () {
    ...
    var tenderCategoriesValues = getTenderCategoriesValues();
     ...
     $('#RFP_Spending').text(tenderCategoriesValues.FRP.Value + " $");
     $('#Auction_Spending').text(tenderCategoriesValues.Auction.Value + " $");
     $('#RFI_Spending').text(tenderCategoriesValues.RFI.Value + " Request");
     $('#RFQ_Spending').text(tenderCategoriesValues.RFQ.Value + " $");
    ...
    var myChart = new Chart(ctx, {
        type: 'pie',
        data: {
            labels: ["Request for information", "Tenders", "Bids", "Quotes"],
            datasets: [{
                backgroundColor: [
                    "#0eb795",
                    "#26879b ",
